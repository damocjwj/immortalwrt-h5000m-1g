...
define Build/h5000m-gpt
	$(TOPDIR)/scripts/h5000m-gpt --sectors $(H5000M_EMMC_SECTORS) --output $@ --part primary
endef

H5000M_EMMC_SECTORS := 488636416
# U-Boot reads the complete external-data FIT into RAM before booting it.  The
# H5000M's 4 GiB DDR permits a 1 GiB FIT alongside the 1 GiB kernel
# decompression window.  Recovery is constrained by its dedicated GPT
# partition.  The rest of the 16 GiB production partition is deliberately
# reserved for F2FS overlay.
H5000M_BOOT_FIT_MAX_SIZE := 1g
H5000M_RECOVERY_FIT_MAX_SIZE := 256m

define Build/h5000m-gpt-backup
	$(TOPDIR)/scripts/h5000m-gpt --sectors $(H5000M_EMMC_SECTORS) --output $@ --part backup
endef

define Build/h5000m-1g-gpt
	$(TOPDIR)/scripts/h5000m-1g-gpt --sectors $(H5000M_1G_EMMC_SECTORS) --output $@ --part primary
endef
...
define Build/h5000m-1g-gpt-backup
	$(TOPDIR)/scripts/h5000m-1g-gpt --sectors $(H5000M_1G_EMMC_SECTORS) --output $@ --part backup
endef
...
define Device/hiveton-h5000m
  ...
  DEVICE_DTS_DIR := ../dts
  DEVICE_DTC_FLAGS := --pad 4096
  DEVICE_DTS_LOADADDR := 0xf0000000
  # The MediaTek target profile enables generic VPN/container/tunnel tools.
  # H5000M uses the MTK-only HNAT console with BBR and excludes unrelated
  # acceleration engines, legacy iptables, VPN, container and unrelated
  # storage tools.  Keep only the fixed internal-eMMC ext4 mount toolchain.
  DEVICE_PACKAGES := -kmod-phy-aquantia -mtkhqos_util \
	-shellsync -kmod-macvlan \
	-kmod-ipt-nat6 -kmod-ipt-nat \
	-kmod-netlink-diag -kmod-ipt-physdev -kmod-nf-ipvs \
	-kmod-veth -kmod-wireguard -kmod-ipt-fullconenat \
	-kmod-fs-btrfs -luci-app-ttyd -ttyd -kmod-bonding \
	-kmod-udptunnel4 -kmod-udptunnel6 \
	-mtd -ubi-utils \
	-iptables-mod-fullconenat \
	fitblk blkid block-mount e2fsprogs \
	mt798x-2p5g-phy-firmware-internal wireless-regdb \
	kmod-mediatek_hnat kmod-warp kmod-mt_wifi_cmn kmod-mt_wifi7 \
	kmod-mt_hwifi kmod-mtk_pci kmod-mtk_wed kmod-connac_if \
	kmod-mt7992 wifi-dats mtwifi-cfg luci-app-mtwifi-cfg \
	kmod-gpio-button-hotplug kmod-leds-gpio \
	kmod-crypto-hw-safexcel \
	kmod-usb3 luci-app-modem sms-tool quectel-CM-5G \
	...
	kmod-hwmon-pwmfan fancontrol luci-app-fancontrol luci-app-turboacc-mtk
  KERNEL_LOADADDR := 0x60000000
  KERNEL := kernel-bin | gzip
  KERNEL_INITRAMFS := kernel-bin | lzma | \
	fit lzma $$(KDIR)/image-$$(firstword $$(DEVICE_DTS)).dtb with-initrd | \
	pad-to 64k | check-size $$(H5000M_RECOVERY_FIT_MAX_SIZE)
  KERNEL_INITRAMFS_SUFFIX := -recovery.itb
  IMAGES := sysupgrade.itb
  # production is a 16 GiB GPT partition; the FIT stays below the U-Boot RAM
  # loading limit and the remaining space is available to the F2FS overlay.
  IMAGE_SIZE := $$(CONFIG_TARGET_ROOTFS_PARTSIZE)m
  IMAGE/sysupgrade.itb := append-kernel | \
	fit gzip $$(KDIR)/image-$$(firstword $$(DEVICE_DTS)).dtb external-with-rootfs | \
	pad-rootfs | pad-to 64k | append-metadata | check-size $$(H5000M_BOOT_FIT_MAX_SIZE)
  ARTIFACTS := emmc-gpt.bin emmc-gpt-backup.bin emmc-preloader.bin emmc-bl31-uboot.fip
  ARTIFACT/emmc-gpt.bin := h5000m-gpt
  ARTIFACT/emmc-gpt-backup.bin := h5000m-gpt-backup
  ARTIFACT/emmc-preloader.bin := mt7987-bl2 emmc-ddr4-4bg
  ARTIFACT/emmc-bl31-uboot.fip := mt7987-bl31-uboot hiveton_h5000m-emmc
  SUPPORTED_DEVICES += hiveton,h5000m
endef

define Device/hiveton-h5000m-1g
  $(Device/hiveton-h5000m)
  DEVICE_MODEL := H5000M (1G/8G)
  DEVICE_DTS := mt7987a-hiveton-h5000m-1g
  DEVICE_DTS_LOADADDR := 0x4ff00000
  ...
  KERNEL_INITRAMFS := kernel-bin | lzma | \
	fit lzma $$(KDIR)/image-$$(firstword $$(DEVICE_DTS)).dtb with-initrd | \
	pad-to 64k | check-size $$(H5000M_1G_RECOVERY_FIT_MAX_SIZE)
  IMAGE_SIZE := $$(CONFIG_TARGET_ROOTFS_PARTSIZE)m
  IMAGE/sysupgrade.itb := append-kernel | \
	fit gzip $$(KDIR)/image-$$(firstword $$(DEVICE_DTS)).dtb external-with-rootfs | \
	pad-rootfs | pad-to 64k | append-metadata | check-size $$(H5000M_1G_BOOT_FIT_MAX_SIZE)
  ARTIFACT/emmc-gpt.bin := h5000m-1g-gpt
  ARTIFACT/emmc-gpt-backup.bin := h5000m-1g-gpt-backup
  ARTIFACT/emmc-preloader.bin := mt7987-bl2 emmc-comb
  ARTIFACT/emmc-bl31-uboot.fip := mt7987-bl31-uboot hiveton_h5000m-1g-emmc
  SUPPORTED_DEVICES := hiveton,h5000m-1g
endef
